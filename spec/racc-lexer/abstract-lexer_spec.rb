...
describe AbstractLexer do
  ...
  context 'Creation and initialization' do
    ...
    it 'should be in the proper initial state' do
      ...
      [position_in_line_state, token_recognition_state].should == [:at_line_start, :waiting_for_input]
    end

    it 'should ignore identations by default' do
      subject.significant_indentation.should be_false
    end

    it 'should consider an eol as a non token by default' do
      subject.eol_as_token.should be_false
    end
    
    it 'should refuse to scan in absence of input text' do
      lambda { subject.next_token() }.should raise_error(LexicalError, "No input text was provided.")
    end
  ...
  context 'Providing the input text' do  
    it 'should accept the input as a String argument' do
      lambda { subject.input = SampleText }.should_not raise_error
    end 
    
    it 'should accept the input as a StringScanner argument' do
      lambda { subject.input = StringScanner.new(SampleText) }.should_not raise_error
    end   
   

    it 'should have its low-level scanner correctly initialized' do
      subject.input = SampleText
      subject.scanner.should be_kind_of(StringScanner)
      subject.scanner.string.should == SampleText
    end
    
    
    it 'should be in ready state to recognize token(s) in the input stream' do
      subject.input = StringScanner.new(SampleText)
      token_recognition_state = subject.current_state_name(:token_recognition)
      token_recognition_state.should == :ready
    end 
