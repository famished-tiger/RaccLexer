...
module MiniTest::Assertions
  ...
  # Assert values of one or more engine attribute.
  # [anEngine] a LexerEngine under testing.
  # [attributes] a Hash with pairs: attribute name => expected value
  def assert_attributes(anEngine, attributes)
...
module MiniTest::Expectations
  ...
  infect_an_assertion :assert_attributes, :must_satisfy, :reverse
end # module
...
class TestLexerEngine < MiniTest::Unit::TestCase
  ...
  def test_input=()
    # Case 1: input argument is a String
    inputs = [ @sample_text, StringScanner.new(@sample_text) ]
    inputs.each do |inp|
      instance = RaccLexer::LexerEngine.new
      instance.input = inp

      # Is the engine in the proper state machine state?
      assert_in_state(instance, [:at_line_start, :ready])

      # A scanner is associated with the input text
      refute_nil instance.scanner
      assert instance.scanner.string == @sample_text

      # Position is set to begin
      assert instance.lineno == 1
      assert instance.line_offset == 0

      # attribute lexeme initialized to empty string
      assert_empty instance.lexeme
    end
  end
...
SAMPLE
  ...
  it "should scan multiple lines of text" do
    ...
    actual_token.must_equal :token
    subject.must_satisfy({ lexeme: '1234',
      lineno: 1, line_offset: 0 })
    
    actual_token = subject.scan(/\.+/)
    actual_token.must_equal :eol
    subject.must_satisfy({ lineno: 1, line_offset: 17 }) # Position just after eol

    actual_token = subject.scan(/\.+/)
    actual_token.must_equal :indentation
    subject.must_satisfy({ lineno: 2, line_offset: 17 })

    actual_token = subject.scan(/\d+/)
    actual_token.must_equal :token
    subject.must_satisfy({ lexeme: '5678',
      lineno: 2, line_offset: 17 })
    
    actual_token = subject.scan(/\.+/)
    actual_token.must_equal :eol
    subject.must_satisfy({ lineno: 2, line_offset: 39 })
    
    actual_token = subject.scan(/\S+/)
    actual_token.must_equal :token
    subject.must_satisfy({ lexeme: '90',
      lineno: 3, line_offset: 39 })

    actual_token = subject.scan(/\.+/)
    actual_token.must_equal :eol
    subject.must_satisfy({ lineno: 3, line_offset: 42 })

    actual_token = subject.scan(/.+/)
    actual_token.must_equal :eos
    subject.must_satisfy({ lineno: 4, line_offset: 42 })       
  end
...
SAMPLE
  ...
  it "should scan multiline lexemes" do
    ...
    actual_token = subject.scan(/\.+/)
    actual_token.must_equal :eol
    subject.must_satisfy({ lineno: 1, line_offset: 22})
    
    actual_token = subject.scan(@doc_string_pattern)
    actual_token.must_equal :token
    expected_lexeme = <<-EXPECTED
"""
  A very very long
  boring and
  bland text 
"""
EXPECTED
    expected_lexeme.chop!
    subject.must_satisfy({ lexeme: expected_lexeme, 
      lineno: 6, line_offset: 72})

    
    actual_token = subject.scan(/\.+/)
    actual_token.must_equal :eol
    subject.must_satisfy({ lineno: 6, line_offset: 76})

    
    actual_token = subject.scan(/\S+/)
    actual_token.must_equal :token
    subject.must_satisfy({ lexeme: '890', 
      lineno: 7, line_offset: 76})

    actual_token = subject.scan(/\S+/)
    actual_token.must_equal :token
    subject.must_satisfy({ lexeme: '*', 
      lineno: 7, line_offset: 76})

    actual_token = subject.scan(/\.+/)
    actual_token.must_equal :eol
    subject.must_satisfy({ lineno: 7, line_offset: 82})
    
    actual_token = subject.scan(/\.+/)
    actual_token.must_equal :eos
    subject.must_satisfy({ lineno: 8, line_offset: 82})    
  end
